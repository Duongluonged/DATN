...
const TABS = ["Tất cả", "Quản trị viên", "Nhà tuyển dụng", "Lập trình viên"];
  ...
  return (
    <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ borderBottom: "1px solid #e2e8f0" }}>
            {["HỌ VÀ TÊN", "EMAIL", "VAI TRÒ", "TRẠNG THÁI", "THAO TÁC"].map(h => (
        ...
        <tbody>
          {USERS.map((u) => {
            const rc = ROLE_COLORS[u.roleKey];
            return (
              <tr key={u.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                <td style={{ padding: "11px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 30, height: 30, borderRadius: "50%", background: `linear-gradient(135deg,${u.color[0]},${u.color[1]})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{u.initials}</div>
                    <div>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: "#111827" }}>{u.name}</div>
                      <div style={{ fontSize: 10.5, color: "#6b7280" }}>{u.id}</div>
                    </div>
                  </div>
                </td>
                <td style={{ padding: "11px 16px", fontSize: 12, color: "#475569" }}>{u.email}</td>
                <td style={{ padding: "11px 16px" }}>
                  <span style={{ fontSize: 10.5, fontWeight: 600, padding: "3px 8px", borderRadius: 6, background: rc.bg, color: rc.text }}>{u.role}</span>
                </td>
                <td style={{ padding: "11px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#111827" }}>
                    <div style={{ width: 7, height: 7, borderRadius: "50%", background: u.active ? "#22c55e" : "#ef4444", boxShadow: u.active ? "0 0 6px rgba(34,197,94,0.25)" : "none" }} />
                    {u.active ? "Hoạt động" : "Bị khóa"}
                  </div>
                </td>
                <td style={{ textAlign: "left", padding: "10px 16px" }}>
                    <button><Pen size={20} /></button>
                    <button style={{ padding: "12px" }}><Trash size={20} /></button>
                </td>
      ...
      <div style={{ padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid #e2e8f0" }}>
        <div style={{ fontSize: 11.5, color: "#6b7280" }}>Hiển thị 1 – 4 trên tổng số 24,512 người dùng</div>
        <div style={{ display: "flex", gap: 4 }}>
          {["1", "2", "3", "...", "120"].map((p, i) => (
            <button key={i} style={{
              width: 28, height: 28, background: p === "1" ? "#3b82f6" : "#f8fafc",
