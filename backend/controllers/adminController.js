...
exports.getDashboardStats = async (req, res) => {
  ...
  try {
    ...
    // 2. Tổng tin tuyển dụng
    const jobsRes = await pool.request().query(`SELECT COUNT(*) AS total FROM Jobs`);
    ...
    // 6. Top 5 tin tuyển dụng nhiều lượt xem nhất (nếu có ViewCount, không thì xếp theo JobID)
    let hotJobs = [];
    try {
    ...
    // 7. Xu hướng đăng tuyển 6 tháng gần nhất
    let trend = [];
    try {
